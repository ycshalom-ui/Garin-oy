      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center gap-6" role="menubar">
            ...
            <Link 
              to="/admin" 
              className="bg-white text-gray-600 px-3 py-1.5 rounded border border-gray-300 text-sm hover:border-gray-400 hover:text-gray-700 transition focus:outline-none focus:ring-2 focus:ring-gray-400"
              role="menuitem"
            >
              התחבר
            </Link>
          ...
          {/* Mobile Menu Button - LARGE and PROMINENT */}
          <button 
            onClick={() => setIsOpen(!isOpen)} 
            className="lg:hidden p-4 rounded-xl bg-green-700 hover:bg-green-800 transition focus:outline-none focus:ring-4 focus:ring-green-500 text-white shadow-2xl border-2 border-green-600"
            aria-label={isOpen ? 'סגור תפריט' : 'פתח תפריט'}
            aria-expanded={isOpen}
            aria-controls="mobile-menu"
          >
            <div className="flex flex-col items-center gap-1">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3} aria-hidden="true">
                {isOpen ? (
              ...
              <span className="text-xs font-bold tracking-wide">{isOpen ? 'סגור' : 'תפריט'}</span>
            </div>
          </button>
        </div>
      </div>
        ...
        <>
          ...
          >
            ...
            <div className="p-4 space-y-2 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 140px)' }}>
              ...
              {/* Login Button - less prominent */}
              <Link 
                to="/admin" 
                className="flex items-center gap-3 py-2.5 px-4 bg-white text-gray-600 border border-gray-300 hover:border-gray-400 hover:text-gray-700 rounded-lg transition mt-4"
                role="menuitem"
                onClick={() => setIsOpen(false)}
              >
                <span className="text-sm">התחבר</span>
              </Link>
            </div>
